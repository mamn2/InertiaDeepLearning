...
class TabQPolicy(QPolicy):
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        currentState = self.discretize(state)
        nextState = self.discretize(next_state)
        qVals = self.model[currentState + (action, )]

        if done:
            target = reward
        else:
            target = reward + self.gamma * max(self.model[nextState + (0,)], self.model[nextState + (1,)])

        self.model[currentState + (action,)] = qVals + self.lr * (target - qVals)

        return (qVals - target) ** 2
...
if __name__ == '__main__':
    ...
    policy = TabQPolicy(env, buckets=(1, 1, 6, 18), actionsize=actionsize, lr=0.12, gamma=args.gamma)
